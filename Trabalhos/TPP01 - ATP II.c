...
short checkOne(float *p1, float *p2, float *p3);
...
void main(){
    ...
    do {
		...
		case 4:			                
			...
			scanf("%f", &k);
			if(checkOne(coefs1, coefs2, coefs3))
...
//Checar se há pelo menos um polinômio introduzido 
short checkOne(float *p1, float *p2, float *p3){
	int i = 0;
	short ok = 0;
	while(i < 20){
	...
	if(!ok){
		...
		if(!ok){
			...
			if(!ok){
				printf("\nNenhum polinômio foi introduzido!");
				return 0;
			}		
